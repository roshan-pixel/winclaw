...
function parseRetryAfterSeconds(text, response) {
  var payload = parseDiscordApiErrorPayload(text);
  var retryAfter =
...
function formatRetryAfterSeconds(value) {
  ...
  var rounded = value < 10 ? value.toFixed(1) : Math.round(value).toString();
  return "".concat(rounded, "s");
}
function formatDiscordApiErrorText(text) {
  ...
  var payload = parseDiscordApiErrorPayload(trimmed);
  if (!payload) {
    var looksJson = trimmed.startsWith("{") && trimmed.endsWith("}");
  ...
  var message =
    typeof payload.message === "string" && payload.message.trim()
      ...
      : "unknown error";
  var retryAfter = formatRetryAfterSeconds(
    typeof payload.retry_after === "number" ? payload.retry_after : undefined,
  );
  return retryAfter ? "".concat(message, " (retry after ").concat(retryAfter, ")") : message;
}
var DiscordApiError = /** @class */ (function (_super) {
  __extends(DiscordApiError, _super);
  function DiscordApiError(message, status, retryAfter) {
...
function fetchDiscord(path_1, token_1) {
  return __awaiter(this, arguments, void 0, function (path, token, fetcher, options) {
    var fetchImpl, retryConfig;
    var _this = this;
    var _a;
    if (fetcher === void 0) {
      fetcher = fetch;
    ...
    return __generator(this, function (_b) {
      ...
      retryConfig = (0, retry_js_1.resolveRetryConfig)(
        ...
        (0, retry_js_1.retryAsync)(
          function () {
            return __awaiter(_this, void 0, void 0, function () {
              var res, text, detail, suffix, retryAfter;
              return __generator(this, function (_a) {
                switch (_a.label) {
                  case 0:
                    return [
                      4 /*yield*/,
                      fetchImpl("".concat(DISCORD_API_BASE).concat(path), {
                        headers: { Authorization: "Bot ".concat(token) },
                      }),
                    ];
                  case 1:
                    res = _a.sent();
                  ...
                  case 2:
                    ...
                    retryAfter = res.status === 429 ? parseRetryAfterSeconds(text, res) : undefined;
          ...
          __assign(__assign({}, retryConfig), {
            ...
            shouldRetry: function (err) {
              return err instanceof DiscordApiError && err.status === 429;
            },
