...
- **Gateway won't start:** Check that `WinClaw_API_KEYS` is set in your environment and that port 18789 is not in use.
- **MCP tools not found:** Ensure you ran `pip install -r mcp-servers/requirements.txt` and `npm install` in `mcp-servers/`.
- **WhatsApp bridge unreachable:** Verify that `whatsapp_log_bridge_server.py` is running and listening on port 5001.
- **Permission errors on Windows:** Run your terminal as Administrator for registry and service tools.
